...
const matchRequestSchema = new Schema(
  {
    ...
    expiresAt: {
      type: Date,
      default: () => new Date(Date.now() + Number(process.env.MATCH_REQUEST_TTL_MS)),
      index: true,
    },
  },
  {
...
matchRequestSchema.pre("validate", function validateDistanceRange() {
  if (this.minDistanceMeters > this.maxDistanceMeters) {
    throw new Error("minDistanceMeters cannot be greater than maxDistanceMeters");
  }
});
